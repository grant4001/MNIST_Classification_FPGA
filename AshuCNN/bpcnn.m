function cnn=bpcnn(cnn,yy, BITS)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if cnn.layers{cnn.no_of_layers}.type == 'f'
       ...
       temp3 = cnn.layers{cnn.no_of_layers}.er{1} * ( cnn.layers{cnn.no_of_layers-1}.outputs)' / size(cnn.layers{cnn.no_of_layers}.er{1}, 2);
       cnn.layers{cnn.no_of_layers}.dW = s_quantize(temp3, BITS);
         temp4 = mean( cnn.layers{cnn.no_of_layers}.er{1}, 2);
         cnn.layers{cnn.no_of_layers}.db = s_quantize(temp4, BITS);
...
for i=cnn.no_of_layers-1:-1:1    
    ...
    elseif cnn.layers{i}.type == 'f'
       ...
       cnn.layers{i}.er{1} = applyactfunccnn(cnn.layers{i}.outputs,cnn.layers{i}.act_func, 1, cnn.layers{i}.er{1} ); 
        temp = cnn.layers{i}.er{1} * ( cnn.layers{i-1}.outputs)' / size(cnn.layers{i}.er{1}, 2);
        cnn.layers{i}.dW = s_quantize(temp, BITS);
        temp2 = mean( cnn.layers{i}.er{1}, 2);
        cnn.layers{i}.db = s_quantize(temp2, BITS);
