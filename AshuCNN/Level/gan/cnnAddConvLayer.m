...
%         cnn.layers{l_current}.K(:,:,k) = ones(cnn.layers{l_current}.kernel_height);
%     end
% end
% 
% 
...
for i = 1 : n_channel
    for j=1: prev_layer_no_featuremaps
        k = k + 1;
%       cnn.layers{l_current}.K(:,:,k)= 0.5*rand(cnn.layers{l_current}.kernel_height,cnn.layers{l_current}.kernel_width)-0.25;
        cnn.layers{l_current}.K(:,:,k) = [0.01,0.02;0.03,0.04];
%        cnn.layers{l_current}.K(:,:,k) = [1,2;3,4];
    end
end
...
cnn.layers{l_current}.S_dK          = ones(cnn.layers{l_current}.kernel_width, cnn.layers{l_current}.kernel_height);
cnn.layers{l_current}.S_db          = 1;
cnn.layers{l_current}.v_dK          = zeros(cnn.layers{l_current}.kernel_width, cnn.layers{l_current}.kernel_height, cnn.layers{l_current}.no_featuremaps * cnn.layers{l_current}.prev_layer_no_featuremaps);
cnn.layers{l_current}.v_db          = zeros(1, cnn.layers{l_current}.no_featuremaps);
