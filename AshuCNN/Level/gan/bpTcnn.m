...
for l_current = (Tcnn.no_of_layers) : -1 : 2
    ...
    %********************************************************
    if l_current == Tcnn.no_of_layers  
        if Tcnn.loss_func == 'cros' %cross_entropy'
            if Tcnn.layers{Tcnn.no_of_layers}.act_func == 'sigm'
                ...
                Tcnn.loss = sum(er1(:))/size(er1,3); %loss over all examples

        else
            er1 = er.^2;
            Tcnn.loss = sum(er1(:))/(2*size(er1,3)); %loss over all examples
    ...
    %********************************************************
    % Error of the prev layers                              *
    %********************************************************
    
        switch Tcnn.layers{l_current+1}.type
        % **********************************************************************
        % Error back propagation: pos layer is Transpose layer
        % **********************************************************************
            case 't'
                %************************************************************
        ...
        % **********************************************************************
        % Error back propagation: pos layer is BatchNorm layer
        % **********************************************************************
            case 'b'
                %********************************************************
                % Convert er_pos into 1D                                *          
                %********************************************************
                %                    
                %  [1 3  '+' [5 7  ==>  [1:8]'
                %   2 4]      6 8]  
                ...
                for i_fMap = 1 : Tcnn.layers{l_current+1}.no_featuremaps
                    er_temp = reshape(Tcnn.layers{l_current+1}.er{i_fMap},[],1);
                    er_pos = [er_pos;er_temp];
                end
                
                %********************************************************
                % Error calculation                                     *          
                %********************************************************
                X       = Tcnn.layers{l_current+1}.X;                
                mu      = Tcnn.layers{l_current+1}.mu;
                vari    = Tcnn.layers{l_current+1}.vari;
                gamma   = Tcnn.layers{l_current+1}.gamma;
                dout    = er_pos;
                er      = batchNorm_backward(X, mu, vari, gamma, dout);
                
                %********************************************************
                % Reshape the 1-d error in to 'T-conv'                  *          
                %********************************************************
                
                %****************************
                % Reshape the error from    *
                % from 1D to 'T-conv' shape *
                %****************************                   
                %     [1:8]' ==> [1 3  '+' [5 7
                %                 2 4]      6 8]  
                % 
                ...
                for i_fMap = 1 : Tcnn.layers{l_current}.no_featuremaps          
                    Tcnn.layers{l_current}.er{i_fMap} = ...
                        zeros(fMap_width, fMap_width, batch_size); % Initilization
                    Tcnn.layers{l_current}.er{i_fMap} = ...
                        reshape(er((i_fMap-1)*no_element_fMap+1 : i_fMap*no_element_fMap), ...
...
%                                 rot90(input,2), 'valid');
                            dK_temp = convn(Tcnn.layers{l_current}.er{i_dK}, ...
                                flipdim(flipdim(flipdim(input,1),2),3), 'valid');       % XXX can't use rot90() for 3-D convn!!!
        ...
        % **********************************************************************
        case 'f'    
            if sum(strcmp(Tcnn.layers{l_current+1}.type, {'t', 'b'}))
                %***************************************************************
        ...
        case 'b'
            if strcmp(Tcnn.layers{l_current+1}.type, 't')
                ...
                er = [];
                for i_fMap = 1 : Tcnn.layers{l_current}.no_featuremaps
                    er_temp = reshape(Tcnn.layers{l_current}.er{i_fMap},[],1);
                    er = [er;er_temp];
                end
                %********************************************************
                ...
                %********************************************************
                dout    = er;
                X_norm  = Tcnn.layers{l_current}.X_norm;
%                 Tcnn.layers{l_current}.dgamma = mean(dout .* X_norm);
%                 Tcnn.layers{l_current}.dbeta  = mean(dout);
                Tcnn.layers{l_current}.dgamma = sum(dout .* X_norm)/batch_size;
                Tcnn.layers{l_current}.dbeta  = sum(dout)/batch_size;
