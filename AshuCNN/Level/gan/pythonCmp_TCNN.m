...
% Case IIII: with batch normalization
% fMap_w = 5;
% 
% Tcnn = cnnAddFCLayer(Tcnn, fMap_w*fMap_w*40, 'rect'); % 
% Tcnn.layers{Tcnn.no_of_layers}.featuremap_width = fMap_w;
...
% Tcnn = cnnAddBatchNormLayer(Tcnn);
% Tcnn = cnnAddTransConvLayer(Tcnn, 1, [4 4], 2, 'valid','sigm');
Tcnn = cnnAddFCLayer(Tcnn, 4 , 'rect');
Tcnn.layers{Tcnn.no_of_layers}.featuremap_width = 2;
Tcnn = cnnAddTransConvLayer(Tcnn, 1, [2 2], 1, 'valid','rect');
Tcnn = cnnAddTransConvLayer(Tcnn, 1, [2 2], 1, 'valid','sigm');
%*******************************************************************************

% noise = ones(noise_width,noise_height,batch_size*n_epoch_xxx);
% noise(:,:,1) = reshape([0:3],2,[])';
% noise(:,:,2) = reshape([4:7],2,[])';
% noise(:,:,3) = reshape([8:11],2,[])';
% noise(:,:,4) = reshape([12:15],2,[])';
noise(:,:,1) = [0:3];
noise(:,:,2) = [4:7];
noise(:,:,3) = [8:11];
noise(:,:,4) = [12:15];
train_img_sel = ones(4,4,batch_size*n_epoch_xxx);
% figure;
% hold on;
tic
