...
switch padding 
    case 'valid'
        Tcnn.layers{l_current}.featuremap_width = strides*prev_layer_featuremap_width + Tcnn.layers{l_current}.kernel_width - strides;
    case 'same'
...
%             Tcnn.layers{l_current}.kernel_width)-0.5;
       % Tcnn.layers{l_current}.K(:,:,k)= ones(Tcnn.layers{l_current}.kernel_height, ...
         %   Tcnn.layers{l_current}.kernel_width);
        Tcnn.layers{l_current}.K(:,:,k) = [0.01,0.02;0.03,0.04];

    end
end

% Tcnn.layers{l_current}.b = rand([n_channel 1]) - 0.5;
Tcnn.layers{l_current}.b = zeros([n_channel 1]);
