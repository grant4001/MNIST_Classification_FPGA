...
if cnn.loss_func == 'cros' %cross_entropy'
    if cnn.layers{cnn.no_of_layers}.act_func == 'sigm'
        eps = 1e-12;        
        er1 = -1.*sum((label.*log(cnn.layers{cnn.no_of_layers}.outputs+eps) + (1-label).*log(1-cnn.layers{cnn.no_of_layers}.outputs+eps)), 1);
    else
...
elseif cnn.layers{cnn.no_of_layers}.type == 'c'
    ...
    for i=1:cnn.layers{cnn.no_of_layers}.no_featuremaps
        for j=1:cnn.layers{cnn.no_of_layers-1}.no_featuremaps
            ...
            kk = kk+1;
%             cnn.layers{cnn.no_of_layers}.dK(:,:,kk) = mean(zz,2);
             cnn.layers{cnn.no_of_layers}.dK(:,:,kk) = zz/size(cnn.layers{cnn.no_of_layers}.er{i},3);% XXX modified by zc 3.31
        end
        ...
        switch cnn.layers{i+1}.type
            ...
            case 'f'
                ...
                for j=1:cnn.layers{i}.no_featuremaps
                    ...
                    sz1 = sz(1)*sz(2);
                    cnn.layers{i}.er{j} = reshape(er(: , j), sz(1), sz(2), 1);
