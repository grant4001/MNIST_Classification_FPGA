...
for i = 2 : cnn.no_of_layers
    switch cnn.layers{i}.type
        case'f'
           ...
           cnn.layers{i}.v_dW = Beta * cnn.layers{i}.v_dW + cnn.layers{i}.dW;
           cnn.layers{i}.v_db = Beta * cnn.layers{i}.v_db + cnn.layers{i}.db;
        ...
        case {'c', 't'}        
           ...
           for j = 1:cnn.layers{i}.no_featuremaps
                for k = 1:cnn.layers{i-1}.no_featuremaps
                    ...
                    cnn.layers{i}.v_dK(:,:,kk) = Beta * cnn.layers{i}.v_dK(:,:,kk) + cnn.layers{i}.dK(:,:,kk);
                ...
                cnn.layers{i}.v_db(j) = Beta * cnn.layers{i}.v_db(j) + cnn.layers{i}.db(j);
...
%       cnn.layers{i}.b = quantize_level_clip(cnn.layers{i}.b, bit_level, paramRange_ori);
        ...
        %cnn.layers{i}.b = quantize_round_clip(cnn.layers{i}.b, paramRange_ori * 2^bit_scale);
        case {'b'}
            cnn.layers{i}.gamma = cnn.layers{i}.gamma - cnn.learning_rate*( cnn.layers{i}.dgamma);
            cnn.layers{i}.beta = cnn.layers{i}.beta - cnn.learning_rate*( cnn.layers{i}.dbeta);
        case {'p'}
